...
db.prepare('INSERT INTO users (email, password_hash, name) VALUES (?, ?, ?)').run('admin@els-corner.com', hash, 'Admin');

const insertProgram = db.prepare(`INSERT INTO programs (title, description, category, format, level, duration, price, icon, featured)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`);
...
const insertTestimonial = db.prepare(`INSERT INTO testimonials (name, course, rating, quote, initials, featured)
  VALUES (?, ?, ?, ?, ?, ?)`);
...
insertSetting.run('whatsapp_number', '+62895613163308');
insertSetting.run('site_name', JSON.stringify({ id: "El's Corner", en: "El's Corner" }));
insertSetting.run('site_description', JSON.stringify({ id: 'Penyedia kursus bahasa Inggris', en: 'English language course provider' }));
insertSetting.run('contact_email', 'info@els-corner.com');
insertSetting.run('contact_phone', '(021) 1234-5678');
insertSetting.run('address', JSON.stringify({ id: "El's Corner Tower, Lantai 4\nJl. Pendidikan No. 10\nJakarta Pusat 10110", en: "El's Corner Tower, 4th Floor\nJl. Pendidikan No. 10\nCentral Jakarta 10110" }));

const insertStudent = db.prepare('INSERT INTO students (name, email, phone, notes) VALUES (?, ?, ?, ?)');
const s1 = insertStudent.run('Budi Santoso', 'budi@example.com', '081234567890', 'IELTS target band 7.5. Prefers morning sessions.');
const s2 = insertStudent.run('Siti Rahmawati', 'siti@example.com', '081298765432', 'Business English for corporate presentations.');
const s3 = insertStudent.run('David Wong', 'david@example.com', '087812345678', 'General English, intermediate level.');

const insertPkg = db.prepare('INSERT INTO student_packages (student_id, package_name, total_sessions, remaining_sessions, start_date, end_date, status) VALUES (?, ?, ?, ?, ?, ?, ?)');
insertPkg.run(s1.lastInsertRowid, 'IELTS Academic Prep Boost', 8, 5, '2026-05-01', '2026-07-01', 'active');
insertPkg.run(s1.lastInsertRowid, 'General English Mastery', 12, 0, '2026-01-15', '2026-04-15', 'completed');
insertPkg.run(s2.lastInsertRowid, 'Business Communication Pro', 10, 3, '2026-04-10', null, 'active');
insertPkg.run(s3.lastInsertRowid, 'General English Mastery', 12, 1, '2026-03-01', '2026-06-01', 'active');

const insertSession = db.prepare('INSERT INTO session_records (student_package_id, session_date, notes) VALUES (?, ?, ?)');
// Budi's IELTS sessions (3 completed out of 8, 5 remaining)
insertSession.run(1, '2026-05-02', 'Introduction to IELTS format. Speaking Part 1 practice.');
insertSession.run(1, '2026-05-09', 'Listening Section 1 & 2 strategies. Map labeling.');
insertSession.run(1, '2026-05-16', 'Writing Task 1 - Line graphs and bar charts.');
// Budi's completed General English (all 12 done, now 0 remaining)
for (let i = 0; i < 12; i++) {
  insertSession.run(2, `2026-0${1 + Math.floor(i/4)}-${String(5 + (i % 4) * 7).padStart(2, '0')}`, `Session ${i + 1} - General English`);
}
// Siti's Business English sessions (7 completed out of 10, 3 remaining)
insertSession.run(3, '2026-04-12', 'Business email etiquette and formal writing.');
insertSession.run(3, '2026-04-19', 'Presentation structure and signposting language.');
insertSession.run(3, '2026-04-26', 'Negotiation phrases and roleplay.');
insertSession.run(3, '2026-05-03', 'Telephone and video call communication.');
insertSession.run(3, '2026-05-10', 'Cross-cultural communication differences.');
insertSession.run(3, '2026-05-17', 'Report writing and data analysis.');
insertSession.run(3, '2026-05-24', 'Mock presentation with feedback.');
// David's General English sessions (11 completed out of 12, 1 remaining)
for (let i = 0; i < 11; i++) {
  insertSession.run(4, `2026-0${3 + Math.floor(i/4)}-${String(1 + (i % 4) * 7).padStart(2, '0')}`, `Session ${i + 1} - General English`);
}
